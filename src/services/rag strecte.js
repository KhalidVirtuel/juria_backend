...
export async function ragAnswer({ 
  ...
  // ✅ FILTRE DE PERTINENCE pour legal_question
  // Si les chunks ne contiennent que des mentions superficielles du sujet, répondre directement
  if (intention === 'legal_question' && hasDocuments) {
    // Extraire les mots-clés importants de la question (noms de codes/lois)
    const legalKeywords = question.match(/code\s+(?:de\s+)?(?:commerce|travail|civil|famille|pénal|procédure|douane|investissement)/gi) || [];
    
    if (legalKeywords.length > 0) {
      const combinedText = results.map(r => r.payload?.text || '').join(' ').toLowerCase();
      
      for (const keyword of legalKeywords) {
        const kw = keyword.toLowerCase();
        // Chercher si le keyword apparaît comme CONTENU principal (pas juste une note de bas de page)
        // Une note de bas de page est courte, commence par un numéro, etc.
        const chunks = results.map(r => r.payload?.text || '');
        const hasSubstantialContent = chunks.some(chunk => {
          const lower = chunk.toLowerCase();
          if (!lower.includes(kw.replace(/code\s+(de\s+)?/i, ''))) return false;
          // Vérifier que c'est du contenu substantiel (plus de 200 chars avec le keyword)
          const idx = lower.indexOf(kw.replace(/code\s+(de\s+)?/i, ''));
          const surrounding = chunk.substring(Math.max(0, idx-100), idx+200);
          // Si c'est une note de bas de page (contient "Bulletin Officiel", "dahir n°", etc.)
          const isFootnote = /bulletin officiel|dahir n°|b\.o\.|promulguée par/i.test(surrounding);
          return !isFootnote;
        });
        
        if (!hasSubstantialContent) {
          console.log(`[RAG] 🚫 Sujet "${keyword}" absent de la base (seulement mentions superficielles)`);
          return {
            answer: `Je n'ai pas trouvé cette information dans les documents juridiques disponibles. Le ${keyword} n'est pas encore vectorisé dans notre base de connaissances. Veuillez ajouter ce document dans la Base de connaissances.`,
            timelineEntries: [],
            documentTags: [],
            intention,
          };
