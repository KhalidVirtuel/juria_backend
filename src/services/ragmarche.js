...
export async function ragAnswer({ 
  ...
  if (intention === 'legal_question' && hasDocuments) {
    const notInBase = [
      { detect: /code\s+de\s+commerce/i,      keywords: ['commerçant', 'fonds de commerce', 'société en nom collectif', 'billet à ordre'], label: 'Code de commerce' },
      { detect: /code\s+du\s+travail/i,         keywords: ['salarié', 'licenciement', 'syndicat', 'grève', 'heures supplémentaires'],        label: 'Code du travail' },
      { detect: /soci[eé]t[eé]s?\s+anonymes?/i, keywords: ['conseil d\'administration', 'assemblée générale ordinaire', 'commissaire aux comptes', 'dividende'], label: 'loi sur les sociétés anonymes' },
    ];
    ...
    for (const { detect, keywords, label } of notInBase) {
      if (!detect.test(question)) continue;
      if (vectorizedFiles.some(f => detect.test(f))) continue;

      const hasSubstantialContent = keywords.some(kw => {
        const idx = allChunksText.indexOf(kw.toLowerCase());
        if (idx === -1) return false;
        const surrounding = allChunksText.substring(Math.max(0, idx - 150), idx + 300);
        const isFootnote = /bulletin officiel|dahir n°\s*1-|b\.o\.|\(\*\)/.test(surrounding);
        return !isFootnote;
      });

      if (!hasSubstantialContent) {
        console.log(`[RAG] 🚫 "${label}" absent de la base vectorisée`);
        return {
          answer: `Je n'ai pas trouvé cette information dans les documents juridiques disponibles. **${label}** n'est pas encore dans notre base de connaissances. Veuillez l'ajouter via la section "Base de connaissances".`,
          timelineEntries: [],
          documentTags: [],
          intention,
        };
