...
// ✅ Scroll scan Qdrant pour trouver un article spécifique
async function scrollSearchForArticle(articleNum, docFilter = null) {
  try {
    const collection = cfg.qdrant?.collection || 'company_knowledge_fr';
    const qdrantUrl = cfg.qdrant?.url || 'http://qdrant:6333';
    
    // Pattern pour matcher "Article 10" comme titre (début de ligne ou après saut de ligne)
    const articleTitlePattern = new RegExp(`(?:^|\\n)\\s*Article\\s+${articleNum}\\b`, 'i');
    // Pattern plus large pour les mentions
    const articleMentionPattern = new RegExp(`Article\\s+${articleNum}\\b`, 'i');
    
    let offset = null;
    let titleChunks = [];  // Chunks avec l'article comme titre
    let mentionChunks = []; // Chunks qui mentionnent l'article
    let iterations = 0;
    ...
    while (iterations < 30) {
      ...
      for (const pt of points) {
        ...
        if (articleTitlePattern.test(text)) {
          titleChunks.push({ payload: pt.payload, score: 1.0 });
        } else if (articleMentionPattern.test(text)) {
          mentionChunks.push({ payload: pt.payload, score: 0.5 });
        }
      }
      
      // S'arrêter si on a trouvé des chunks avec le titre de l'article
      if (titleChunks.length >= 2) break;
    ...
    // Trier par chunk_index pour retourner les articles dans l'ordre du document
    titleChunks.sort((a, b) => (a.payload?.chunk_index || 0) - (b.payload?.chunk_index || 0));
    mentionChunks.sort((a, b) => (a.payload?.chunk_index || 0) - (b.payload?.chunk_index || 0));
    
    const found = [...titleChunks, ...mentionChunks.slice(0, 3)];
    
    // ✅ Marquer si plusieurs articles avec le même numéro existent
    if (titleChunks.length > 1) {
      found._hasDuplicates = true;
      found._duplicateCount = titleChunks.length;
      console.log(`[RAG] ⚠️ Article ${articleNum} apparaît ${titleChunks.length} fois dans la base`);
    }
    
    console.log(`[RAG] 📜 Scroll Article ${articleNum}: ${titleChunks.length} titre(s), ${mentionChunks.length} mention(s)`);
    return found;
  } catch (e) {
    console.warn('[RAG] ⚠️ Scroll search failed:', e.message);
    return [];
  }
}
...
async function smartRAGSearch(question, intention, metaFilter = {}) {
  ...
  try {
    ...
    // ✅ Si article spécifique, vérifier si présent dans résultats initiaux
    if (articleMatch) {
      const articleNum = articleMatch[1];
      ...
      if (withArticle.length === 0) {
        console.log(`[RAG] 🔄 Article ${articleNum} absent, recherche élargie à 50...`);
        results = await searchSimilar(qVec, 50, filter);
      }
    }

    // Prioriser les résultats contenant l'article exact
    if (articleMatch && results.length > 0) {
      ...
      if (withArticle.length > 0) {
        ...
        results = [...withArticle, ...withoutArticle.slice(0, 5)];
      } else {
        // FALLBACK : scroll scan - filtrer par document si la question mentionne un code spécifique
        console.log(`[RAG] 🔄 Fallback scroll search pour Article ${articleNum}...`);
        
        // Détecter quel fichier est mentionné dans la question pour filtrer le scroll
        const normalize = s => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
        const qNorm = normalize(question);
        const allFiles = [...new Set(results.map(r => r.payload?.path || r.payload?.filename || '').filter(Boolean))];
        let scrollDocFilter = null;
        
        // Chercher quel fichier correspond à la question
        for (const f of allFiles) {
          const fNorm = normalize(f);
          if (qNorm.includes('pénal') || qNorm.includes('penal')) {
            if (fNorm.includes('penal')) { scrollDocFilter = f; break; }
          } else if (qNorm.includes('famille')) {
            if (fNorm.includes('famille')) { scrollDocFilter = f; break; }
          } else if (qNorm.includes('juridiction') || qNorm.includes('financier')) {
            if (fNorm.includes('juridiction')) { scrollDocFilter = f; break; }
          }
        }
        
        // Si pas de filtre trouvé via question, chercher dans tous les fichiers vectorisés
        if (!scrollDocFilter) {
          const vectorizedPaths = [...new Set(results.map(r => r.payload?.path || '').filter(Boolean))];
          if (vectorizedPaths.length === 1) scrollDocFilter = vectorizedPaths[0];
        }
        
        console.log(`[RAG] 🔍 Scroll avec filtre fichier: ${scrollDocFilter || 'aucun'}`);
        const scrollResults = await scrollSearchForArticle(articleNum, scrollDocFilter);
        if (scrollResults.length > 0) {
          console.log(`[RAG] ✅ Scroll: ${scrollResults.length} chunks avec Article ${articleNum}`);
          results = [...scrollResults, ...results.slice(0, 5)];
          // ✅ Stocker l'info de doublons pour le prompt
          if (scrollResults._hasDuplicates) {
            results._articleDuplicateNote = `⚠️ NOTE IMPORTANTE: L'Article ${articleNum} apparaît ${scrollResults._duplicateCount} fois dans ce document (numérotation répétée dans différentes parties du code). Tu afficheras OBLIGATOIREMENT la version trouvée dans le contexte ET tu ajouteras cette note à la fin de ta réponse: "⚠️ Attention : L'article ${articleNum} apparaît plusieurs fois dans ce document avec des contenus différents selon la partie du code. Si ce n'est pas l'article recherché, précisez le contexte (ex: peines, infractions, procédure...)."`; 
          }
        } else {
          console.log(`[RAG] ⚠️ Article ${articleNum} introuvable même par scroll`);
        }
      }
    }
...
- <strong>texte</strong> pour gras (INTERDIT: **texte**)
...
🎨 FORMATAGE:
- <strong>texte</strong> pour le gras (JAMAIS **texte**)
...
🎨 FORMATAGE:
- Utilise <strong>texte</strong> pour le gras (JAMAIS **texte** ni -texte-)
...
export async function ragAnswer({ 
  ...
  // ✅ FILTRE DE PERTINENCE GLOBAL
  if (intention === 'legal_question' && hasDocuments) {
    const legalDocMatch = question.match(
      /(?:code\s+[\w\u00C0-\u024F]{2,}(?:\s+[\w\u00C0-\u024F]{2,}){0,4}|loi\s+(?:n°|sur|relative|portant)\s+[\w\u00C0-\u024F\s]{2,40})/i
    );
    if (legalDocMatch) {
      const mentionedDoc = legalDocMatch[0].trim();
      const vectorizedFiles = results.map(r => (r.payload?.path || r.payload?.filename || '').toLowerCase());
      // Normaliser les accents pour comparer avec les noms de fichiers (ex: pénal → penal)
      const normalize = s => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
      
      const docKeywords = normalize(mentionedDoc)
        .replace(/^(code|loi)\s+(de\s+la?\s+|du\s+|des\s+|n°\s*[\d-]+\s*)?/i, '')
        .split(/\s+/)
        .filter(w => w.length > 3 && !['dans','pour','avec','les','des','aux','sur','par'].includes(w));
      
      const normalizedFiles = vectorizedFiles.map(f => normalize(f));
      const isVectorized = normalizedFiles.some(f => docKeywords.some(kw => f.includes(kw)));
      if (!isVectorized) {
        const allChunksText = results.map(r => r.payload?.text || '').join('\n');
        const hasSubstantialContent = docKeywords.some(kw => {
          const lower = allChunksText.toLowerCase();
          const idx = lower.indexOf(kw);
          if (idx === -1) return false;
          const surrounding = allChunksText.substring(Math.max(0, idx - 200), idx + 400);
          const isFootnote = /bulletin officiel|dahir n°\s*1-\d|b\.o\.|\(\*\)|précitée/i.test(surrounding);
          const isSubstantial = /article\s+\d+|chapitre\s+[iivx\d]/i.test(surrounding);
          return isSubstantial && !isFootnote;
        });
        if (!hasSubstantialContent) {
          console.log(`[RAG] 🚫 "${mentionedDoc}" absent de la base vectorisée`);
          return {
            answer: `Je n'ai pas trouvé cette information dans les documents juridiques disponibles. **${mentionedDoc}** n'est pas encore dans notre base de connaissances. Veuillez l'ajouter via la section "Base de connaissances".`,
            timelineEntries: [], documentTags: [], intention,
          };
        }
      }
    }
  ...
  // ✅ Injecter note de doublons si applicable
  if (results._articleDuplicateNote) {
    messages.push({ role: 'system', content: results._articleDuplicateNote });
    console.log('[RAG] ⚠️ Note doublons injectée dans le prompt');
  }
