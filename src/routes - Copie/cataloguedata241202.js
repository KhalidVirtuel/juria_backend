...
const upload = multer({ storage }); // ✅ Correct
...
const uploadDOC = multer({ storage: storageDOC }); // ✅ Correct : storage: storageDOC
...
catalogueDataRouter.post('/upload', authRequired, uploadDOC.single('file'), async (req, res) => {
  try {
    ...
    const filePath = req.file.path; // ✅ Maintenant req.file.path existe !
