...
// Multer storage (écrit directement sur disque dans BASE_DIR_DOC)
...
const upload_DOC = multer({ storage_DOC });
...
catalogueDataRouter.post('/upload', authRequired, upload_DOC.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'file required (multipart/form-data)' });
    ...
    if (!description || !description.trim()) {
      ...
      return res.status(400).json({ error: 'description is required' });
        ...
        console.log('yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy', { name, description, file: req.file.originalname });
     console.log('zzzzzzzzzzzzzzzzzzzzzzzzzz', req.file);
    const filePath = req.file.path;
    const basename = path.basename(filePath);
    console.log('zzzzzzzzzzzzzzzzzzzzzzzzzz 11111111', filePath);
    console.log('zzzzzzzzzzzzzzzzzzzzzzzzzz 222222222222', basename);
