...
foldersRouter.post('/:id/timeline', authRequired, async (req, res, next) => {
  try {
    const userId = req.user.uid;
    const folderId = Number(req.params.id);
    const { title, type, description, date, conversationId, showInCalendar } = req.body || {}; // ✅ AJOUTER conversationId
  ...
  try {
    ...
    res.json({
      entries: items.map(ev => ({
        id: String(ev.id),
        folderId: String(ev.folderId),
        conversationId: ev.conversationId ? String(ev.conversationId) : undefined,
        title: ev.label,
        description: ev.note ?? '',
        type: (ev.type || 'EVENT').toLowerCase(),
        date: ev.date.toISOString(),
        createdAt: ev.createdAt.toISOString(),
        showInCalendar: ev.showInCalendar ?? true,
      })),
    });
  } catch (e) { next(e); }
});
...
foldersRouter.put('/timeline/:eventId', authRequired, async (req, res, next) => {
  try {
    const userId = req.user.uid;
    const id = Number(req.params.eventId);
    const { title, type, description, date, conversationId, showInCalendar } = req.body || {};
    ...
    const updated = await prisma.timelineEvent.update({
      ...
      data: {
        ...
        ...(showInCalendar !== undefined && { showInCalendar: Boolean(showInCalendar) }),
      },
    });
    ...
    res.json({
      entry: {
        ...
        showInCalendar: updated.showInCalendar ?? true,
      },
    });
  } catch (e) { next(e); }
});
    ...
    const payload = {
      folders: folders.map(f => ({
        ...
        timeline:    (evByFolder[f.id]   || []).map(t => ({
          ...
          showInCalendar: t.showInCalendar ?? true,
        })),
        conversations: [],
      })),
    };
