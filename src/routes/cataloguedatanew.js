 ...
 * Supprime fichier + métadonnées
 */
catalogueDataRouter.delete('/filescatalogue/:basename', authRequired, async (req, res) => {
  try {
    // Essayer dans BASE_DIR_DOC d'abord
    const pDoc = safeJoin(BASE_DIR, req.params.basename);
    const metaPathDoc = path.join(BASE_DIR, `${req.params.basename}.meta.json`);
...
catalogueDataRouter.delete('/files/:basename', authRequired, async (req, res) => {
  try {
    ...
    // Sinon essayer dans BASE_DIR
    const p = safeJoin(BASE_DIR_DOC, req.params.basename);
    const metaPath = path.join(BASE_DIR_DOC, `${req.params.basename}.meta.json`);
