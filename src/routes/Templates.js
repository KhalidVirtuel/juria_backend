...
// ✅ Répertoire catalogue (même que cataloguedata.js)
const CATALOGUE_DIR = path.resolve(process.cwd(), 'data/uploads/document_fr');
...
templatesRouter.post('/upload', authRequired, upload.single('file'), async (req, res, next) => {
  try {
    ...
    // ✅ Copier le fichier dans le catalogue pour vectorisation
    try {
      const catalogueFilename = req.file.filename; // même nom horodaté
      const cataloguePath = path.join(CATALOGUE_DIR, catalogueFilename);

      // Copier le fichier .docx dans le répertoire catalogue
      fs.copyFileSync(req.file.path, cataloguePath);

      // Créer les métadonnées catalogue (.meta.json)
      const metadataPath = path.join(CATALOGUE_DIR, `${catalogueFilename}.meta.json`);
      const metadata = {
        name: name.trim(),
        description: (description || `Template ${type} - ${subtype}`).trim(),
        filename: catalogueFilename,
        originalName: req.file.originalname,
        uploadedAt: new Date().toISOString(),
        size: req.file.size,
        mimetype: req.file.mimetype,
        source: 'template',         // ✅ Marqué comme provenant des templates
      ...
      console.log(`✅ Template aussi ajouté au catalogue: ${cataloguePath}`);
    } catch (catalogueError) {
      // Ne pas bloquer si la copie catalogue échoue
      console.error('⚠️ Failed to copy template to catalogue:', catalogueError);
    }
...
templatesRouter.delete('/:id', authRequired, async (req, res, next) => {
  try {
    ...
    try {
      const cataloguePath = path.join(CATALOGUE_DIR, template.filename);
      const catalogueMetaPath = path.join(CATALOGUE_DIR, `${template.filename}.meta.json`);

      if (fs.existsSync(cataloguePath)) {
        fs.unlinkSync(cataloguePath);
        console.log(`✅ Template supprimé du catalogue: ${cataloguePath}`);
      }
      if (fs.existsSync(catalogueMetaPath)) {
        fs.unlinkSync(catalogueMetaPath);
      }
    } catch (catalogueError) {
      console.error('⚠️ Failed to delete template from catalogue:', catalogueError);
    }
