...
// ===== fileToDTO =====
// ✅ URL HTTP au lieu du chemin filesystem absolu
const fileToDTO = (fullPath, baseDir = BASE_DIR) => {
  const stat = fs.statSync(fullPath);
  const name = path.basename(fullPath);

  // Déterminer le préfixe d'URL selon le répertoire
  const urlPrefix = baseDir === BASE_DIR_DOC
    ? '/api/cataloguedata/download/doc'
    : '/api/cataloguedata/download/cat';
  ...
  return {
    ...
    name,
    size: stat.size,
    url: `${urlPrefix}/${encodeURIComponent(name)}`, // ✅ URL HTTP valide
...
/**
 * GET /api/cataloguedata/download/doc/:filename
 * Télécharger un fichier depuis BASE_DIR_DOC
 */
catalogueDataRouter.get('/download/doc/:filename', authRequired, (req, res) => {
  try {
    const filename = decodeURIComponent(req.params.filename);
    const filePath = safeJoin(BASE_DIR_DOC, filename);
...
/**
 * GET /api/cataloguedata/download/cat/:filename
 * Télécharger un fichier depuis BASE_DIR (catalogue lois)
 */
catalogueDataRouter.get('/download/cat/:filename', authRequired, (req, res) => {
  try {
    const filename = decodeURIComponent(req.params.filename);
    const filePath = safeJoin(BASE_DIR, filename);
