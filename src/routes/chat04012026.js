  ...
  await prisma.conversation.update({
    ...
    try {
      ...
      if (result.answer && typeof result.answer === 'string') {
        const intention = result.intention || 'general'; // ✅ Capturer l'intention
...
// ✅ Synthèse vocale pour intention general
let audioData = null;
if (intention === 'general' && answerText) {
  try {
    console.log('[AUDIO] Generating speech for general intent...');
    
    const mp3 = await openai.audio.speech.create({
      model: 'tts-1',
      voice: 'nova', // Vous pouvez changer: alloy, echo, fable, onyx, nova, shimmer
      input: answerText,
      speed: 1.0
    });
