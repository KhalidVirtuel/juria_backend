...
// 🔧 util: normaliser le champ note -> { description, type }
function parseNote(note) {
  if (!note) return { description: '', type: 'event' };
  try {
    // note peut être un string JSON ou un texte simple
    const v = typeof note === 'string' ? JSON.parse(note) : note;
    if (typeof v === 'object' && v !== null) {
      return {
        description: v.description ?? v.desc ?? '',
        type: (v.type ?? 'event').toLowerCase(),
      };
    }
    // si c’est juste un texte
    return { description: String(v), type: 'event' };
  } catch {
    // pas du JSON -> renvoyer brut
    return { description: String(note), type: 'event' };
  }
}
...
foldersRouter.post('/', authRequired, async (req, res, next) => {
  try {
    ...
    const folder = await prisma.folder.create({
      data: { userId, name, description: description ?? null, color: color ?? null }, // <---
...
foldersRouter.get('/', authRequired, async (req, res, next) => {
  try {
    ...
    // 1) On ne mélange pas select et include
    const folders = await prisma.folder.findMany({
      where: { userId },
      orderBy: { createdAt: 'desc' },
      include: {
        documents: true, // OK car relation existe
        // ❌ PAS de timelineEvents / deadlines ici car pas de relation dans le modèle Folder
      },
    });

    // 2) On récupère deadlines + timeline events par requêtes séparées, puis on groupe
    ...
    const deadlinesByFolder = new Map();
    for (const d of deadlines) {
      if (!deadlinesByFolder.has(d.folderId)) deadlinesByFolder.set(d.folderId, []);
      deadlinesByFolder.get(d.folderId).push(d);
    }
    const eventsByFolder = new Map();
    for (const t of events) {
      if (!eventsByFolder.has(t.folderId)) eventsByFolder.set(t.folderId, []);
      eventsByFolder.get(t.folderId).push(t);
    }

    // 3) Map → shape frontend (réutilise tes helpers si tu les as)
    const payload = {
      folders: folders.map((f) => ({
        id: String(f.id),
        userId: String(f.userId),
        ...
        color: f.color ?? '#3b82f6',
        createdAt: f.createdAt.toISOString(),
        attachments: (f.documents || []).map(doc => ({
          id: String(doc.id),
          folderId: String(doc.folderId),
          name: doc.title,
          type: (doc.mimeType || 'document').toLowerCase(),
        ...
        timeline: (eventsByFolder.get(f.id) || []).map(t => {
          const { description, type } = parseNote(t.note);
          return {
            id: String(t.id),
            folderId: String(t.folderId),
            title: t.label,                 // tu gardes le titre
            description,                    // ✅ plus de JSON brut, seulement "Procedure 01"
            type,                           // ex: 'procedure' si présent, sinon 'event'
            date: t.date.toISOString(),
            createdAt: t.createdAt.toISOString(),
          };
        }),
        deadlines: (deadlinesByFolder.get(f.id) || []).map(d => ({
          id: String(d.id),
          folderId: String(d.folderId),
          title: d.title,
          description: d.note ?? '',
          dueDate: d.dueDate.toISOString(),
          priority: 'medium',   // adapte si tu gères une priorité
          status: 'pending',    // adapte si tu gères un statut
          createdAt: d.createdAt.toISOString(),
        })),
        documents: [],      // (documents IA générés si tu en as)
        conversations: [],  // (si tu veux enrichir plus tard)
      })),
    };
console.log("payload.timeline")
console.log(payload.folders[0])
console.log(payload.folders[1])
console.log(payload.folders[2])
  ...
  try {
    ...
    const folder = await prisma.folder.findFirst({
      ...
      select: {
        ...
        documents: { select: { id: true, folderId: true, title: true, path: true, size: true, mimeType: true, createdAt: true } },
      },
      include: {
        documents: true,
        timelineEvents: { orderBy: { date: 'desc' } },
  ...
  try {
    ...
    const payload = {
      folder: {
        ...
        attachments: (folder.documents || []).map(doc => ({
          id: String(doc.id),
          folderId: String(doc.folderId),
          name: doc.title,
          type: (doc.mimeType || 'document').toLowerCase(),
...
foldersRouter.put('/:id', authRequired, async (req, res, next) => {
  try {
    ...
    return res.json({ folder: toFolderDTO(f) });
  } catch (e) { next(e); }
});
