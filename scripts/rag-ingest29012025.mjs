...
const DEFAULT_KB_DIR = path.resolve(process.cwd(), 'data/uploads/knowledge_fr');

// Parse des arguments: supporte "node rag-ingest.mjs <dir>" ou "--dir=..."
// et garde l’ancien flag --dedupe
const argv = process.argv.slice(2);
let overrideDir = null;
for (let i = 0; i < argv.length; i++) {
  const a = argv[i];
  if (a.startsWith('--dir=')) overrideDir = a.slice('--dir='.length);
  else if (a === '--dir') overrideDir = argv[i + 1];
  else if (!a.startsWith('--')) {
    // Premier argument positionnel non-flag = dossier
    if (!overrideDir) overrideDir = a;
  }
}
// Normalise le chemin (remplace backslashes Windows)
if (overrideDir) {
  overrideDir = overrideDir.replace(/\\/g, '/');
}
const KB_DIR = path.resolve(process.cwd(), overrideDir || DEFAULT_KB_DIR);

// flags existants
const ARGS = new Set(argv);
const DEDUPE = ARGS.has('--dedupe');

console.log(`📁 KB_DIR: ${KB_DIR}  ${DEDUPE ? '(dedupe ON)' : ''}`);
